...
class Brain:
    ...
    def train(self):
        ...
        q_current = q_orig.detach() - (
            config.MAX_MASK_CONST * m
        )  # unavailable actions do not influence the max
        ...
        p_matrix.scatter_(
            1, a_max, (1 - self.epsilon) + (self.epsilon / a_count)
        )  # p(a* | s)
        ...
        terminal = torch.ones(
            batch_len, dtype=torch.float32
        )  # terminal flag (0 when terminal)
        ...
        for i in range(1, max_ep_len):
            ...
            q[idxs] = (
                r[idxs]
                + terminal[idxs] * config.GAMMA * q_estm[idxs + 1]
                + terminal[idxs]
                * config.GAMMA
                * c[idxs + 1]
                * (q[idxs + 1] - q_perf[idxs + 1])
            )
    ...
    def _get_batch(env, size):
        ...
        q_c = np.full(
            (size, config.TERMINAL_ACTIONS), config.REWARD_INCORRECT, dtype=np.float32
        )
    ...
    def update_epsilon(self, epoch):
        if epoch >= config.PI_EPSILON_EPOCHS:
            ...
            self.epsilon = (
                config.PI_EPSILON_START
                + epoch
                * (config.PI_EPSILON_END - config.PI_EPSILON_START)
                / config.PI_EPSILON_EPOCHS
            )
